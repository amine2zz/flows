...
function initDB() {
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS the216_users (
...
function seedProducts($pdo) {
    ...
    $products = array(
        ...
        array('cap-216',           'Casquette 216',            'Casquette structuree brodee THE216. Ajustable, unisexe.',                      35.00, 'Accessoires', null,          '#1a1a1a',  'TU',           100, 0, 4),
    ...
    $stmt = $pdo->prepare('INSERT INTO products (slug,name,description,price,category,badge,color_hex,sizes,stock,featured,sort_order) VALUES (?,?,?,?,?,?,?,?,?,?,?)');
    foreach ($products as $p) $stmt->execute($p);
}

function seedAdmin($pdo) {
    $exists = $pdo->query("SELECT COUNT(*) FROM the216_users WHERE role='admin'")->fetchColumn();
    if ($exists) return;
    $pdo->prepare("INSERT INTO the216_users (name,email,phone,password,role) VALUES (?,?,?,?,?)")
        ->execute(array('Admin','admin@the216.tn','00000000', password_hash('Admin216!', PASSWORD_DEFAULT),'admin'));
}
...
function getAuthUser() {
    ...
    try {
        $u = getDB()->prepare('SELECT id,name,email,phone,city,address,role FROM the216_users WHERE id=? LIMIT 1');
...
function statusLabel($s) {
    $map = array('pending'=>'En attente','confirmed'=>'Confirme','shipped'=>'Expedie','delivered'=>'Livre','cancelled'=>'Annule');
